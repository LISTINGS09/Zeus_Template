// Dog Tag Respawn - v1.0 by 2600K
// Players must bring ACE Dog Tags back to any medical vehicle in order to respawn dead players.

// Should Dog Tag respawn be enabled? Disabled by default.
if (isNil "f_param_dogTagRespawn") then { f_param_dogTagRespawn = false };

// Do only medics get the action trigger?
if (isNil "f_param_dogTagOnlyMedics") then { f_param_dogTagOnlyMedics = true };
...
if (!f_param_dogTagRespawn || 
	(f_param_dogTagOnlyMedics && getNumber (configFile >> "CfgVehicles" >> (typeOf player) >> "attendant") == 0 && !(player getUnitTrait "Medic")) ||
...
zeu_dog_check = {
	...
	private _inReviveArea = count (_unit nearObjects 8 select { (_x getVariable ["ace_medical_medicClass", getNumber (configFile >> "CfgVehicles" >> typeOf _x >> "attendant")]) > 0 && !(_x isKindOf "Man") }) > 0;
		...
		// Find player from dog tag name from matching dog tag item index.
		{
			...
			if (_tagIndex >= 0) then {
				...
				if (count _foundPlayer > 0) then {
					...
					[["<t size='2'>Your Dog Tag was recovered - Replacement Enlisted</t>","PLAIN DOWN", 2, true, true]] remoteExec ["TitleText", _target];
