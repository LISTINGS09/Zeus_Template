...
if (_flagMarker in allMapMarkers) then {
	...
	f_obj_spawnFlag addAction ["<t color='#0080FF'>Virtual Garage</t>", { 
		if (!isNil "ZEU_fnc_StartVirtualGarage") then { 
			[] spawn ZEU_fnc_StartVirtualGarage;
		} else { 
			private _scr = [] execVM "f\misc\f_virtualGarage.sqf"; 
			waitUntil { scriptDone _scr };
			
			if (isNil "ZEU_VG_LocalObject") then { 
				systemChat "[VG] Check the briefing if you want to set a Virtual Garage spawn point"; 
			} else {
				[] spawn ZEU_fnc_StartVirtualGarage; 
			}; 
		}; 
	}, nil, 1.4, true, true, "", "missionNamespace getVariable ['f_param_virtualGarage',0] != 0 OR serverCommandAvailable '#kick'"];
