...
{if(_x != (vehicle _x)) then {if(!((vehicle _x) in _vehG)) then {_vehG pushBack (vehicle _x)}}}forEach units _group;
sleep 0.1;
if(count _vehG > 0) then
{
	while{_i < count _vehG} do
	{
		...
		{
			{if((_x != (driver (_vehG select _i))) && (_x != (gunner (_vehG select _i))) && (_x != (commander (_vehG select _i)))) then {_allCargo pushBack [_x,(_vehG select _i)]}}forEach _cargoA;
		};
		_i = _i + 1;
	};
};
