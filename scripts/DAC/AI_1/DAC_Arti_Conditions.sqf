...
{
	_uArray pushBack format["%1",side _leader];
	switch (format["%1",side _leader]) do
	{
		case "WEST":	{if(DAC_Res_Side == 1) then {_uArray pushBack "GUER"}};
		case "EAST":	{if(DAC_Res_Side == 0) then {_uArray pushBack "GUER"}};
		case "GUER":	{
							if(DAC_Res_Side == 1) then {_uArray pushBack "WEST"};
							if(DAC_Res_Side == 0) then {_uArray pushBack "EAST"};
						};
		default {_i = 3;_a = 0};
	};
	DAC_Arti_Array pushBack _enemyGroup;
	...
	while {_i < 3} do
	{
		switch (_i) do
		{
			case 0:	{
						...
						if(_i < 3) then {{if((_x != vehicle _x) && (!((vehicle _x) in _veh))) then {_veh pushBack vehicle _x}} forEach (units _enemyGroup)};
			...
			case 2:	{
						...
						{
							{
								...
								&& (format["%1",gunner (_x select 0)] != "<NULL-object>")) then
								{
									...
									if(((_x select 0) distance _target) < _maxdistance) then
									{
										...
										{
											_artiVeh pushBack [(_x select 0),(_x select 1)];
											DAC_Arti_Veh pushBack (_x select 0);
										}
										else
										{
						...
						if((_i < 3) && (((count _addcondition) > 0) || ((count _artiVeh) > 0))) then
						{
							...
							{
								_tempV pushBack ((_addcondition select _n) select 0);
								_tempA pushBack ((_addcondition select _n) select 1);
								_n = _n + 1;
							};
												...
												if(!(_artiUnit in DAC_Arti_Veh)) then
												{
													if(!(_artiUnit in _tArray)) then
													{
														if((_condition select 6) >= 1) then
														{
															...
															if(_f >= 0) then
															{
																_artiVeh pushBack [_artiUnit,(_tempA select _f)];
																_tArray pushBack _artiUnit;
															};
														}
														else
														{
		...
		{	
			if(count _artiVeh > (_condition select 4)) then
			{
				while {(count _sArray < (_condition select 4))} do
				{
					...
					{
						_cArray pushBack _val;
						_sArray pushBack (_artiVeh select _val);
						DAC_Arti_Veh pushBack ((_artiVeh select _val) select 0);
					};
				};
			}
			else
			{
				_sArray = _artiVeh;
				{DAC_Arti_Veh pushBack (_x select 0)}forEach _artiVeh;
			};
