...
if(({alive _x} count units _grouponfire) > 0) then
{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						if((_firepos distance _manonfire) < ((DAC_Player_Support select 1) select 3)) then
						{
							DAC_Hit_Groups pushBack _grouponfire;
	...
	if(_action) then
	{
		...
		if(!(_grouponfire in DAC_Call_Groups)) then {DAC_Call_Groups pushBack _grouponfire};
		...
		while{_run} do
		{
			while{_go} do
			{
				...
				{
					_tempgroups pushBack _checkgroup;
					if((count _tempgroups) == (count _TempAll)) then
					{
						_tempgroups pushBack _grouponfire;
						_run = false;
						_go = false
					};
				};
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									[_hgroup] spawn DAC_fWaitForHelp;_DAC_Temp_Groups pushBack _hgroup;
