	...
	if(format["%1",group _unit] != "<NULL-group>") then
	{
		if(!(_gun in DAC_Static)) then
		{
			...
			while {_go == 1} do
			{
				...
				if(_go == 1) then {if(((damage _gun) == 1) || (((count crew _gun) > 0) && (_unit != gunner _gun))) then {_go = 2}};
			...
			if(_go > 0) then
			{
				...
				else
				{
					while {_go == 3} do
					{
						...
						if(_go == 3) then {if(((damage _gun) == 1) || (!(canfire _gun))) then {_go = 4}};
