	...
	switch (_run) do
	{
		...
		case 6:	{
					...
					if(_oldskill != ((getMarkerPos (_marker select 2)) select 1)) then
					{
						...
						_unitwait = "((((_zskill select 7) select (_groupid - 1)) select 0) + (random (((_zskill select 7) select (_groupid - 1)) select 1)))";
						_group setCombatMode (selectRandom _unitcombat); _group setBehaviour (selectRandom _unitbehav);
						_group setFormation (selectRandom _unitform); // _group setSpeedMode (selectRandom _unitspeed); // Intentionally removed?
		...
		case 10:{
					...
					_group setVariable ["dacData",[0,_nextpos],false];
					_group move _nextpos;
					if(!(_group in DAC_In_Building)) then {DAC_In_Building pushBack _group};
					//-----------------------------
					//#movetobuilding
					//-----------------------------
					_go = 1;_kw = 1;
						...
						if(isNull(_veh select 0)) then
						{
							...
							else
							{
								while {_i >= 0} do
								{
									...
									{
										_newveh = _gunU;_gunA pushBack _gunU;_i = -1;
									};
								};
		...
		case 12:{
					...
					if(!(_newveh in DAC_VehicleInUse)) then {DAC_VehicleInUse pushBack _newveh};
					_groupveh pushBack _newveh;
					...
					while {_go > 0} do
					{
						...
						{
							if((({alive _x} count crew _newveh) > 0) || (damage _newveh > 0.8) || (!(canMove _newveh))) then
							{
								_go = 0;_run = 2;
							}
							else
					...
					if(_run == 12) then
					{
						...
						while {_go > 0} do
						{
							...
							{
								if((damage _newveh > 0.8) || (!(canMove _newveh))) then
								{
									_go = 0;_run = 2;
								}
								else
					...
					else
					{
						...
						else
						{
							if(_group in DAC_Hit_Groups) then
							{
								...
								{if((!(side _x == side leader _group)) && (((leader _group) knowsabout _x) > 0) && (alive _x))  then {_callg pushBack _x}} forEach DAC_Fire_Pos;
							...
							while {_go > 0} do
							{
								...
								if(time > _waittime) then
								{
									if(_id == 1) then
									{
										...
										if(!(_group in DAC_Help_Groups)) then {DAC_Help_Groups pushBack _group};
