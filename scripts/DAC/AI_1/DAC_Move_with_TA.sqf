	...
	switch (_run) do
	{
		...
		case 2:	{
					...
					while {_go > 0} do
					{
						...
						else
						{
							...
							// 2600K - Error position: <select (random ((count _wparray) - (1 + >
							//diag_log text format["DAC_Move_with_TA: %1 %2 %3",_wparray, count _wparray, _mz];
		...
		case 4:	{
					...
					while {_go > 0} do
					{
						...
						else
						{
							...
							if((({(vehicle _x) == _x} count (units _group)) == count (units _group)) && (_go == 1)) then {_go = 0;_run = 7};
		...
		case 6:	{
					...
					if(_oldskill != ((getMarkerPos (_marker select 2)) select 1)) then
					{
						...
						_unitwait = "((((_zskill select 7) select (_groupid - 1)) select 0) + (random (((_zskill select 7) select (_groupid - 1)) select 1)))";
						_group setCombatMode (selectRandom _unitcombat); _group setBehaviour (selectRandom _unitbehav);
						_group setFormation (selectRandom _unitform);
		...
		case 13:{
					...
					if(({(vehicle _x) == _x} count (units _group)) > 0) then
					{
						...
						{
							{if((vehicle _x) == _x) then {_x setUnitPos "down"}} forEach units _group;
						}
						else
						{
							...
							{
								{if((vehicle _x) == _x) then {_x setUnitPos "auto"}} forEach units _group;
							};
						};
					};
				};
