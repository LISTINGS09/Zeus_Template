...
switch (_typ) do
{
	case 0:	{
				if(format["%1",_groupA] != "<NULL-group>") then
				{
					...
					{
						call compile format["DAC_GroupPool%1 pushBack _groupA",(_sidearray select _para)];
						{deleteWaypoint [_groupA, _x]} forEach [0,1,2,3];
					};
				};
			};
	case 1:	{
				if((DAC_Com_Values select 0) > 0) then {systemChat format["%1 joining %2",_groupA,_para]};
				sleep 0.1;
