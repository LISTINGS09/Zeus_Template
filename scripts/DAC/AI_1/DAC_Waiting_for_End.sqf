		...
		switch (_run) do
		{
			...
			case 4:	{
						...
						if((DAC_Com_Values select 0) > 0) then
						{
							...
							{
								systemChat format["%1 was deactivated",_group];
							}
							else
							{
								systemChat format["%1 was deleted",_group];
							};
						};
						...
						else
						{
							...
							{_allcampobj pushBack _x} forEach (_camparray select 4);
							{_allcampobj pushBack _x} forEach (_camparray select 6);
							if((count (_camparray select 8)) > 0) then {{_allcampobj pushBack _x} forEach ((_camparray select 8) select 0)};
							...
							else
							{
								...
								if(!(format["%1",_tempbuild] == "<NULL-object>")) then
								{
									if((format["%1",call compile _tempvalue] == format["%1",call compile _tempvalue2]) && (_tempcount > 0)) then
									{
										if(_temptyp == 0) then
										{
											_temparray1 pushBack _i;
											_temparray2 pushBack _i;
										}
										else
										{
											if(_temptyp == 1) then
			...
			case 10:{
						...
						if(!(_building in DAC_Camps_InUse)) then {DAC_Camps_InUse pushBack _building};
						if(((position _shet) select 0) == 0) then {_shet setPos (_building modelToWorld [0,15,0]);systemChat "Bad Spawn Position"};
						DAC_SpawnDummy setPos position _shet;
						call compile format["_grouppara execVM ""scripts\DAC\Scripts\DAC_Group_%1.sqf""",_scriptTyp select (_groupid - 1)];
						...
						{
							(((call compile _dc) select _rdmcamp) select 5) set [1,(((((call compile _dc) select _rdmcamp) select 5) select 1) - 1)];
							if((DAC_Com_Values select 0) > 0) then {systemChat format["%1 was respawned",_group]};
							sleep _rTime;
							if(_building in DAC_Camps_InUse) then {DAC_Camps_InUse = DAC_Camps_InUse - [_building]};
							_run = 0;
						};
			...
			case 11:{
						[_group,_groupside,0] spawn DAC_fRemoveGroup;
						if((DAC_Com_Values select 0) > 0) then {systemChat format["%1 was released from DAC",_group]};
						_run = 0;
					};
			Default	{};
		};
