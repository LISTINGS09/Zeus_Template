...
while {_find1 == 0} do
{
	...
	{
		case 3:	{
					systemChat format["Error connect Camps: %1 is not a valid side value",_ZoneSide];
					_find1 = 2;
				};
		case 2:	{
					if(_countF == 0) then
					{
						//systemChat format["No Camp on side %1 are connected.",_SideTX select _SideID];
					};
					_find1 = 3;
				};
		case 1:	{
																					...
																					else
																					{
																						...
																						else
																						{
																							...
																							if(!((getPosATL _CheckZone select 0) in (((call compile _CampList) select _mn) select 6))) then
																							{
																								...
																								{
																									systemChat format["(%4) DAC connects [%1_Camp_%2] with [Zone_%3]",_SideTX select _SideID,round (getPosATL (((call compile _CampList) select _mn) select 1) select 0),round (getPosATL _CheckZone select 0),_countF];
																								};
																							};
																							_mn = _mn + 1;
																						};
...
sleep (random 1);
[_SideID,_flag]execVM "scripts\DAC\Marker\DAC_CampMarker.sqf";