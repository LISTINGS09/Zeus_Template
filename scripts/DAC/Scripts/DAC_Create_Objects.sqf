...
private [
			"_DAC_WP_Typ","_wpConfig","_CheckRadius1","_CheckRadius2","_checkAreaH","_checkNear","_checkObjH1","_dist","_isLogic","_emptyPos",
...
{		
	...
	{
		_scr = _this execVM "scripts\DAC\Scripts\DAC_Check_ObjScr.sqf";
		...
		if(DAC_Basic_Value != 10) then
		{
			...
			if((_wpc + _DAC_WP_Typ) == 0) then
			{
				...
				[(_this select 0),_DAC_Count_WP,_obj_Nr,_inst,_objArray,(_this select 7)] execVM "scripts\DAC\Scripts\DAC_SimplePos_Objects.sqf";
			}
			else
			{
				if(count _objArray > 1) then
				...
				if(count _this == 8) then
				{
					if(_inst > 0) then
					{
						...
						{
							[(_this select 0),(_DAC_Count_WP - 0.5),_wpc,_DAC_WP_Typ,_obj_Nr,_inst,_objArray,(_this select 7),_dist,objNull,_logA] execVM "scripts\DAC\Scripts\DAC_Create_Objects.sqf";
						}
						else
						{
							[(_this select 0),_DAC_Count_WP,_wpc,_DAC_WP_Typ,_obj_Nr,_inst,_objArray,(_this select 7),_dist,objNull,_logA] execVM "scripts\DAC\Scripts\DAC_Create_Objects.sqf";
						};
				...
				DAC_Obj_Init pushBack format["%1%2",_KiZone,count _this];
				sleep (random 0.5);
				while {_run > 0} do
				{
					switch (_run) do
					{
						case 3:	{
									...
									else
									{
										...
										{
											[_KiZone,10,_DAC_ZoneRadiusX,_DAC_ZoneRadiusY,0] execVM "scripts\DAC\Marker\DAC_ZoneMarker.sqf";
										}
										else
										{
											[_KiZone,10,0,_DAC_PolyCount,0] execVM "scripts\DAC\Marker\DAC_ZoneMarker.sqf";
						...
						case 11:{
									...
									if(_run != 7) then
									{
										...
										{	
											...
											_emptyPos = [_RandomX,_RandomY] findEmptyPosition [0, 100, (_newo select 0)]; 
											if (count _emptyPos > 0) then { _obj = (_newo select 0) createVehicle (_emptyPos select 0);
											} else { _obj = (_newo select 0) createVehicle [_RandomX,_RandomY]; };
											...
											if(!(format["%1",_arrayName] == "")) then {call compile format["%1 pushBack _obj",_arrayName]};
											_TempArray pushBack _obj;
											...
											{
												if(!((_newo select 0) in _debA)) then {_debA pushBack (_newo select 0)};
											}
											else
											{
										...
										else
										{
											...
											if(!(format["%1",_arrayName] == "")) then
											{
												...
												call compile format["%1 pushBack [_RandomX,_RandomY]",_arrayName];
												_TempArray pushBack _obj;
												_wpnr = _wpnr + 1;_chnr = 0;
												if(DAC_Marker > 0) then
												{
						...
						case 12:{
									...
									if((DAC_Marker_Val select 1) == 1) then {if(count _MTemp > 0) then {[_MTemp]execVM "scripts\DAC\Marker\DAC_deleteMarker.sqf"}};
