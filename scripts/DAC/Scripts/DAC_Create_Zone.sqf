...
waitUntil{!isNil "DAC_Com_Values"};
...
else
{
	...
	else
	{
		...
		{
			DAC_CheckZones pushBack _thisZone;
				...
				else
				{
					...
					if(count _zPoly == 0) then
					{
						...
						[
							...
							_myvalue,
							(_this select 6)
						]	execVM "scripts\DAC\Marker\DAC_ZoneMarker.sqf";
					}
					else
					{
						_typ = ((_this select 6) select 0);_mxvalue = ((triggerArea _KiZone) select 0);_myvalue = ((triggerArea _KiZone) select 1);
		...
		DAC_Init_Zone pushBack _randomcount;
		sleep 0.01;
		while {_cu < count _zoneValues} do
		{
			...
			_marker pushBack _ma;
			_cu = _cu + 1;
		};

		DAC_Zones pushBack [_thisZone,_mxvalue,_myvalue,_ZoneMode,_GroupSol,_GroupVeh,_GroupTan,_GroupAir,_ZoneSide,_marker,_zPoly];
		...
		sleep (5 + (random 1));
		if((DAC_Com_Values select 0) > 0) then {if((_ZoneMode select 1) == 1) then {DAC_Inactive_Zones pushBack _KiZone;systemChat format["Zone %1 is deactivated",_thisZone]}};
	};
};