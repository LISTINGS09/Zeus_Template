		...
		switch (_run) do
		{
			...
			case 4:	{
						...
						else
						{
								...
								if((DAC_Com_Values select 0) > 0) then {systemChat format["%1 was deleted (%2MTR)",_UnitTyp,_dist]};
								if(_unit in DAC_VehicleInUse) then {DAC_VehicleInUse pushBack _unit};
								switch (_groupside) do
								{
