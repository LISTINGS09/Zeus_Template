...
private [
			...
			"_rankarray","_vehcargo","_sj","_m","_run","_randomcount","_newgroup","_grouppara","_mp","_TempList","_posarray",
			...
			_m = "";_run = 1;_randomcount = random 999;_newgroup = group player;_vehicle = "";_startpos = [0,0,0];_soldierT = "";
...
if(((getMarkerSize _ma) select 0) == 4) exitWith {};
...
while {_run > 0} do
{
	switch (_run) do
	{
		...
		case 4:	{
					...
					if(getNumber (configFile >> "CfgVehicles" >> (typeOf _vehc) >> "transportSoldier") > 1) then {_vehcargo = 1} else {if(!(_group in DAC_All_Groups)) then {DAC_All_Groups pushBack _group}};
		...
		case 5:	{
					...
					if((((getMarkerSize _ma) select 0) == 6) && (!(DAC_InSpawn)) && ((DAC_Com_Values select 0) > 0) && ((getDir DAC_StartDummy) > 170)) then {systemChat format["%1 (Air) was activated",_group]};
