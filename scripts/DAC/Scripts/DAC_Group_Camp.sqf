...
private [
			...
			"_flag","_wset","_setDG","_camp","_build","_fire","_startpos","_we","_ab","_ammoarray","_wd","_mf",
			...
			_rankarray = ["PRIVATE","CORPORAL","SERGEANT","LIEUTENANT","CAPTAIN","MAJOR","COLONEL"];_run = 1;
...
if(((getMarkerSize _ma) select 0) == 4) exitWith { DAC_Init_Camps = DAC_Init_Camps - 1; };

DAC_Master_Init = DAC_Master_Init + 1;

waitUntil {sleep 0.2; !DAC_InCreate;};
	...
	switch (_run) do
	{
		...
		case 2:	{
					...
					if((count _TempUnitsB > 0) && (!(((_TempUnitsB select 0) select 0) == "NULL"))) then
					{
						...
						else
						{
							...
							{
								hintc format["Attention,\nin an Unit Config for Camps (%1),\nnot all required entries exist.",_groupside];
								DAC_Basic_Value = 4;publicVariable "DAC_Basic_Value";_run = 0;
							}
							else
							{
								_campScr = (_TempUnitsB select 7);DAC_Init_Group pushBack _randomcount;
								...
								_wpstart = call compile (selectRandom _wparray);
								...
								while {_we < count _wset} do
								{
									...
									_ab setDir ((_wset select _we) select 3);_ammoarray pushBack _ab;
								...
								if(DAC_Marker > 0) then
								{
									if((DAC_Marker_Val select 13) > 0) then
									{
										...
										_mf setMarkerSizelocal [0.8,0.8];_mf setMarkerColorlocal (_DAC_MColor select _groupside);_isM pushBack _mf;
									};
								};
								if((count (call compile format["DAC_GroupPool%1",((_sidearray select 1) select _groupside)])) == 0) then
								{
									_newgroup = call compile format["createGroup [%1,true]",((_sidearray select 0) select _groupside)];
								}
								else
								{
					...
					else
					{
						...
						{
							systemChat "Error DAC_Group_Camp: No valid Camp Config";_run = 0;
						};
					};
				};
		case 3:	{
		...
		case 4:	{
					...
					else
					{
						...
						{
							_ab = ((_wset select _we) select 0) createVehicle [(_wpstart select 0) + ((_wset select _we) select 1),(_wpstart select 1) + ((_wset select _we) select 2),0];
							_ab setdir ((_wset select _we) select 3);_objarray pushBack _ab;
							_tempST = _tempST + [_ab,((_wset select _we) select 4)];_staarray pushBack _ab;
							_ab setPos [(_wpstart select 0) + ((_wset select _we) select 1),(_wpstart select 1) + ((_wset select _we) select 2),0];
							_we = _we + 1;
						};
		...
		case 5:	{
					...
					//#createVEG
					//-----------------------------
					{if((_x isKindOf "Air") || (_x isKindOf "Car") || (_x isKindOf "Tank")) then {DAC_VehicleInUse pushBack _x}} forEach _objarray;
		...
		case 7:	{
					...
					else
					{
						...
						while {_we < count _wset} do
						{
							...
							_eman setTargetAge "UNKNOWN";
							_solarray pushBack _eman;_we = _we + 1;
						};
		...
		case 8:	{
					...
					switch (_groupside) do
					{
						case 0:	{
									...
									sleep 0.01;
									{DAC_East_Units pushBack _x} forEach units _group;_wside = 0;
									if(_cs2 == 0) then {DAC_Spawn_CampsE pushBack [_homezone,_camp,_build,_spawnpro,_spawntyp,[_spawncount,0],_vA]};
								};
						case 1:	{
									...
									sleep 0.01;
									{DAC_West_Units pushBack _x} forEach units _group;_wside = 1;
									if(_cs2 == 0) then {DAC_Spawn_CampsW pushBack [_homezone,_camp,_build,_spawnpro,_spawntyp,[_spawncount,0],_vA]};
								};
						case 2:	{
		...
		case 9	:{
					...
					if(!(_group in DAC_Camp_Groups)) then {DAC_Camp_Groups pushBack _group};
					...
					if((((getMarkerSize _ma) select 0) == 6) && (!(DAC_InSpawn)) && ((DAC_Com_Values select 0) > 0) && ((getDir DAC_StartDummy) > 170)) then {systemChat format["%1 (Camp) was activated",_group]};
					DAC_InSpawn = false;
					...
					{
						if(!(format["%1",_groupvar] == "")) then {call compile format["%1 pushBack _group",_groupvar]};
					}
					else
					{
						[_wside,_camp]execVM "scripts\DAC\Marker\DAC_CampMarker.sqf";
					};
		...
		case 11:{
					...
					else
					{
						...
						]	execVM "scripts\DAC\Scripts\DAC_No_Camp.sqf";
						_run = 0;
					};
				};
		Default{};
