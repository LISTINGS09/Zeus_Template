	...
	switch (_run) do
	{
		...
		case 3:	{
					...
					if(!(DAC_InSpawn)) then
					{
						...
						{
							// 2600K - DAC doesn't always initialise group WP correctly?
							// Error position: <compile (_wparray select ((count _wparra>
							// Error compile: Type Number, expected String
							// Remove the 0 value if present
							if ((_wparray select ((count _wparray) - 1)) isEqualType 0) then { 
								_wparray deleteAt ((count _wparray) - 1); 
							};
		...
		case 9:	{
					...
					_group selectLeader (units _group select 0);if(!(_group in DAC_All_Groups)) then {DAC_All_Groups pushBack _group};
					sleep 0.01;
					...
					if((((getMarkerSize _ma) select 0) == 6) && (!(DAC_InSpawn)) && ((DAC_Com_Values select 0) > 0) && ((getDir DAC_StartDummy) > 170)) then {systemChat format["%1 (Soldier) was activated",_group]};
					if(((getMarkerSize _ma) select 0) <= 4) then
					{
						_run = 10;
					}
