...
private [
			"_unitcount","_vehcount","_wparray","_groupid","_groupside","_groupstart","_groupende","_maxrad","_unit_skill","_emptyPos",
...
while {_run > 0} do
{
	switch (_run) do
	{
		...
		case 3:	{
					...
					else
					{
						...
						else
						{
							...
							_emptyPos = _startpos findEmptyPosition [0, 100,_vehicle]; 
							...
							[_vehc,_delval0,_delval1,2,_ma,_newgroup,_delval2,_groupside] spawn DAC_fDeleteUnit;
							...
							if(_WeapList > 0) then {{[_x,format["%1",_WeapList]] execVM "scripts\DAC\Scripts\DAC_Change_Weapons.sqf"}forEach _crewA;_WeapList = 0};
		...
		case 6:	{
					...
					if((((getMarkerSize _ma) select 0) == 6) && (!(DAC_InSpawn)) && ((DAC_Com_Values select 0) > 0) && ((getDir DAC_StartDummy) > 170)) then {systemChat format["%1 (Tank) was activated",_group]};
					if(((getMarkerSize _ma) select 0) <= 4) then
					{
						_run = 7;
					}
