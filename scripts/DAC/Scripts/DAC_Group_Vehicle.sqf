...
private [
			"_unitcount","_wparray","_groupid","_groupside","_groupstart","_groupende","_maxrad","_unit_skill","_emptyPos",
			...
			"_leaderman","_countu","_countg","_grouppara","_group","_res","_run","_EventList2","_numarray","_WeapList",
			...
			_startid = 0;_homezone = position (call compile _groupzone);_run = 1;_vehiclepara = [];_groupvehicle = objNull;
...
if(((getMarkerSize _ma) select 0) == 4 && !DAC_InSpawn) exitWith {};
	...
	switch (_run) do
	{
		...
		case 2:	{
					...
					else
					{
						...
						{
							hintc format["Attention,\nin an Unit Config for Vehicle (%1),\nno entry exists.",_groupside];
							DAC_Basic_Value = 4;publicVariable "DAC_Basic_Value";_run = 0;
						}
						else
						{
							if(DAC_StartDummy in DAC_Init_Group) then {DAC_Init_Group = DAC_Init_Group - [DAC_StartDummy]};
							DAC_Init_Group pushBack _randomcount;DAC_Init_Values set [0,(DAC_Init_Values select 0) + 1];
					...
					else
					{
						...
						else
						{
							...
							if(_sj != 1) then
							{
								...
								// 2600K - Stop vehicles flipping by finding safe empty pos for them.
								private _emptyPos = _startpos findEmptyPosition [0, 100,_vehicle]; 
								if (count _emptyPos > 0) then {
									_vehc = _vehicle createVehicle _emptyPos;
								} else { 
									_vehc = _vehicle createVehicle _startpos;
								};
								
								[_vehc,_delval0,_delval1,2,_ma,group _leaderman,_delval2,_groupside] spawn DAC_fDeleteUnit;
								DAC_VehicleInUse pushBack _vehc;
		...
		case 6:	{
					...
					if(!(_group in DAC_All_Groups)) then {DAC_All_Groups pushBack _group};{_x setTargetAge "30 MIN"} forEach units _group;
					...
					if((((getMarkerSize _ma) select 0) == 6) && (!(DAC_InSpawn)) && ((DAC_Com_Values select 0) > 0) && ((getDir DAC_StartDummy) > 170)) then {systemChat format["%1 (Veh) was activated",_group]};
					if(((getMarkerSize _ma) select 0) <= 4) then
					{
						_run = 7;
					}
		...
		case 7:	{
					...
					if(count units _newgroup > 0) then
					{
						...
						call compile format["DAC_GroupPool%1 pushBack _newgroup",((_sidearray select 1) select _groupside)];
					};
					sleep 0.5;
					if(count units _group > 0) then
					{
						...
						call compile format["DAC_GroupPool%1 pushBack _group",((_sidearray select 1) select _groupside)];
					};
