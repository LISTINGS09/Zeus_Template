...
while {_run > 0} do
{
	switch (_run) do
	{
		...
		case 3:	{
					...
					if(_DAC_WP_Typ == 1) then {{[_x]execVM "scripts\DAC\Scripts\DAC_Init_Waypoint.sqf"} forEach _ScriptArray};
		...
		case 9:	{
					...
					else
					{
						...
						else
						{
							...
							{
								//diag_log text format["DAC_Init_Waypoint9 [%3] Type: (%1) %2", _x, typeOf _x, _DAC_WP_Typ];	
								if(((format["%1",typeOf _x]) != "LOGIC")
		...
		case 10:{
					...
					{
						//diag_log text format["DAC_Init_Waypoint10 [%3] Type: (%1) %2", _x, typeOf _x, _DAC_WP_Typ];	
						if(((format["%1",typeOf _x]) != "LOGIC")
						&& ((format["%1",typeOf _x]) != "EmptyDetector")
						&& ((format["%1",typeOf _x]) == "" && _DAC_WP_Typ != 1) // If infantry we don't care about non-specific terrain features (trees etc)
						&& (speed _x == 0)) then
		...
		case 13:{
					...
					else
					{
						...
						{
							[1]execVM "scripts\DAC\Scripts\DAC_Create_Groups.sqf";
							execVM "scripts\DAC\Marker\DAC_LinkMarker.sqf";
