...
if(isServer) then
{
	...
	{
		systemChat "Error: DAC_Insert_Group - No valid zone";
		DAC_Release_Action = DAC_Release_Action - 1;
		if(time > 0) exitWith {};
	}
	else
		...
		{						
			...
			while {_run == 0} do
			{
				...
				else
				{
					...
					while {_go} do
					{
						...
						if(_id == 0) then {_wparray pushBack format["%1",_newwp];_go = false} else {_id = 0};
					};
				};
			};
			sleep 0.1;
			...
			else
			{
				while {_run == 0} do
				{
					...
					if(_side == 0) then {{DAC_East_Units pushBack _x} forEach units _group};
					if(_side == 1) then {{DAC_West_Units pushBack _x} forEach units _group};
					if(_side == 2) then
					{
					...
					if((_options select 2) == 0) then {DAC_No_Support pushBack _group};
				...
				if((DAC_Com_Values select 0) > 0) then {systemChat format["%1 was inserted into DAC",_group]};
			};
			DAC_Release_Action = DAC_Release_Action - 1;
		}
		else
		{
			systemChat format["No Waypoints available for group %1 (Type = %2)",_group,_gtyp];
			_group setVariable ["DAC_Excluded", True];
			DAC_Release_Action = DAC_Release_Action - 1;
		};
	};
