...
while {_run > 0} do
{
	switch (_run) do
	{
		...
		case 3:	{
					...
					if(_DAC_WP_Typ == 1) then {{[_x,_DAC_This_Zone]execVM "scripts\DAC\Scripts\DAC_Insert_Waypoint.sqf"} forEach _ScriptArray};
					if(!(_DAC_WP_Typ in DAC_Init_WP)) then {DAC_Init_WP pushBack _DAC_WP_Typ};
							...
							if((_DAC_SelectZone select 0) == format["%1",_DAC_This_Zone]) then
							{
								...
								switch (_DAC_WP_Typ) do
								{
									...
									case 4:	{
												...
												else
												{
													...
													DAC_WP_Pool_H pushBack [_DAC_EnemyZone,_DAC_ZoneMode,0,0,0];
												};
											};
									case 5:	{
												...
												DAC_WP_Pool_B pushBack [_DAC_EnemyZone,_DAC_ZoneMode,0,0,0];
											};
									Default {};
								};
								if(_run == 20) then
								...
								else
								{
									...
									else
									{
										...
										{
											call compile format["DAC_WP_Pool_%1 pushBack [_DAC_EnemyZone,_DAC_ZoneMode,_DAC_GroupSol,_DAC_GroupVeh,_DAC_GroupTan,_DAC_GroupAir,_DAC_ZoneSide,_CheckRadius2]",_DAC_Typ_Nr select (_DAC_WP_Typ - 1)];
											_run = 5;_go = 1;
										};
									};
								};
						...
						switch (_go) do
						{
							...
							case 3:	{
										while{(((getwppos[_log,_wn + 1]) select 0)+((getwppos[_log,_wn + 1]) select 1) != 0)} do
										{
											...
											call compile format["DAC_WP_Pool_%1 pushBack [_checkwp select 0,_checkwp select 1]",_DAC_Typ_Nr select (_DAC_WP_Typ - 1)];
		...
		case 7:	{
					...
					while {_go > 0} do
					{
						...
						else
						{
							...
							{
								// 2600K - Error Zero divisor
								// File C:\Users\2600K\Documents\Arma 3\missions\MissionMaker.tanoa\scripts\DAC\Scripts\DAC_Insert_Waypoint.sqf, line 349
								//diag_log text format ["DAC_Insert_Waypoint: %1 %2", _DAC_EnemyZone, triggerArea _DAC_EnemyZone];
								if((triggerArea _DAC_EnemyZone) select 3 && {count (triggerArea _DAC_EnemyZone) > 3}) then
								{
		...
		case 9:	{
					...
					else
					{
						...
						else
						{
							...
							{
								// 2600K - Trying to allow DAC to select positions on roads.
								//diag_log text format["DAC_Insert_Waypoint9 [%3] Type: (%1) %2", _x, typeOf _x, _DAC_WP_Typ];	
								if(((format["%1",typeOf _x]) != "LOGIC")
		...
		case 10:{
					...
					{
						//diag_log text format["DAC_Insert_Waypoint10 [%3] Type: (%1) %2", _x, typeOf _x, _DAC_WP_Typ];	
						if(((format["%1",typeOf _x]) != "LOGIC")
						&& ((format["%1",typeOf _x]) != "EmptyDetector")
						&& ((format["%1",typeOf _x]) == "" && _DAC_WP_Typ != 1) // If infantry we don't care about non-specific terrain features (trees etc)
						&& (speed _x == 0)) then
					...
					else
					{
						...
						if(_run != 7) then
						{
							...
							if(_DAC_Count_WP == 0) then
							{
								call compile format["DAC_WP_Pool_%1 pushBack [0,0]",_DAC_Typ_Nr select (_DAC_WP_Typ - 1)];
							}
							else
							{
								if(_wpnr < _DAC_Count_WP) then {call compile format["DAC_WP_Pool_%1 pushBack [_RandomX,_RandomY]",_DAC_Typ_Nr select (_DAC_WP_Typ - 1)]};
							};
							if(DAC_Marker > 0) then
							{
		...
		case 12:{
					...
					{
						if(_DAC_WP_Typ == 4) then {DAC_WP_Pool_H pushBack [_DAC_EnemyZone,_DAC_ZoneRadiusX,_DAC_ZoneRadiusY,_countairwp]};
						if(_DAC_WP_Typ == 5) then {DAC_WP_Pool_B pushBack [getPosATL _DAC_EnemyZone,_DAC_ZoneRadiusX,_DAC_ZoneRadiusY,0]};
						_run = 4;
					};
				};
		case 13:{
