	...
	switch (_run) do
	{
		...
		case 2:	{
					...
					else
					{
						...
						{
							DAC_Init_Group pushBack _randomcount;
							...
							if(DAC_Marker > 0) then
							{
								if((DAC_Marker_Val select 13) > 0) then
								{
									...
									_mf setMarkerColorlocal (_DAC_MColor select _groupside);_isM pushBack _mf;
								};
							};
		...
		case 3:	{
					...
					if(_cs2 == 0) then
					{
						DAC_Spawn_CampsE pushBack [_homezone,_camp,_camp,_spawnpro,_spawntyp,[_spawncount,0],[]];
					};
					_run = 7;
				};
		case 4:	{
					...
					if(_cs2 == 0) then
					{
						DAC_Spawn_CampsW pushBack [_homezone,_camp,_camp,_spawnpro,_spawntyp,[_spawncount,0],[]];
					};
					_run = 7;
				};
		case 5:	{
		...
		case 9:	{
					...
					DAC_InCreate = true;
					if((DAC_Com_Values select 0) > 0) then {systemChat "New spawnpoint was activated"};
					_run = 2;
				};
		default {};
	};
