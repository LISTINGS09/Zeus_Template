...
switch (_side) do
{
	case 0:	{
				...
				_addUnit = format["DAC_East_Units pushBack _unit"];
				_addPlayer = format["if(!((call compile _x) in DAC_West_Units)) then {DAC_West_Units pushBack (call compile _x)}"];
				_addLeader = format["DAC_East_Units pushBack _leader"];
	...
	case 1:	{
				...
				_addUnit = format["DAC_West_Units pushBack _unit"];
				_addPlayer = format["if(!((call compile _x) in DAC_East_Units)) then {DAC_East_Units pushBack (call compile _x)}"];
				_addLeader = format["DAC_West_Units pushBack _leader"];
...
{if((!(typeOf vehicle _x == typeOf _x)) && (!(vehicle _x in _vehA))) then {_vehA pushBack (vehicle _x)}} forEach units _group;
...
while {_run > 0} do
{
	switch (_run) do
	{
		...
		case 2:	{
					...
					{
						DAC_Min_Groups pushBack _group;_run = 3;
					};
				};
		case 3:	{
					while {_run == 3} do
		...
		case 6:	{
					...
					while {_gnr < count units _group} do
					{
						...
						if((alive _unit) && (!(_unit == leader _group))) then
						{
							if(_typ == 1) then
							{
								...
								{
									_utyp = typeOf _unit;_dam = damage _unit;_UA pushBack _unit;
									_skill =	[
					...
					if(((DAC_Com_Values select 0) > 0) && (format["%1",group _leader] != "<NULL-group>")) then {systemChat format["%1 was reduced",group _leader]};
					if(!(_group in DAC_Min_Groups)) then {DAC_Min_Groups pushBack _group};
		...
		case 8:	{
					...
					else
					{
						...
						if(((DAC_Com_Values select 0) > 0) && (format["%1",group _leader] != "<NULL-group>")) then {systemChat format["%1 is built up",group _leader]};
						...
						while {_gnr < count _grouparray} do
						{
							...
							else
							{
								...
								_gnr = _gnr + 4;_set pushBack _unit;call compile _addunit;
								if(_magn == 1) then {_US pushBack _unit};
