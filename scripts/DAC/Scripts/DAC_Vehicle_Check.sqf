...
while {time > 0} do
{
	...
	while {_i < count _UnitList} do
	{
		...
		if((_entry in DAC_VehicleInUse)	|| ((damage _entry) == 1)) then
		{
			_i = _i + 1;
		}
		else
			...
			{
				{
					...
					{
						case "WEST":	{if(!(_x in DAC_West_Units)) then {DAC_West_Units pushBack _x}};
						case "EAST":	{if(!(_x in DAC_East_Units)) then {DAC_East_Units pushBack _x}};
						case "GUER":	{
											if(DAC_Res_Side == 0) then
											{
												if(!(_x in DAC_East_Units)) then {DAC_East_Units pushBack _x};
											}
											else
											{
												if(!(_x in DAC_West_Units)) then {DAC_West_Units pushBack _x};
											};
										};
						default			{};
					};
