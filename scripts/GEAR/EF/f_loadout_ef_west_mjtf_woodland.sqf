...
_baseHelmet = ["EF_H_MCH_BasicNet_Olive","EF_H_MCH_BasicNet_Wdl"];
_baseGlasses = [""];

// Vests
_lightRig = ["EF_V_AAV_Scout_Olive"];
_mediumRig = ["EF_V_AAV_Rifleman_Olive"]; 	// default for all infantry classes
_heavyRig = ["EF_V_AAV_Support_Olive"];
...
// LOADOUT: CARABINEER
   ...
   default
   {
		...
		if (true) exitWith {diag_log text format ["[F3] DEBUG (f_loadout_ef_west_mjtf_woodland.sqf): Unit = %1. Gear template %2 does not exist!",_unit,_typeOfUnit]};
   };
